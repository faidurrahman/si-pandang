...
export const ServiceModal: React.FC<ServiceModalProps> = ({ service, onClose, onApply }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-[24px] md:rounded-[32px] w-[92%] md:w-full max-w-[500px] max-h-[85vh] flex flex-col overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
        
        {/* Modal Header */}
        <div className="p-6 md:p-8 pb-4 flex items-start justify-between flex-shrink-0">
          <div className="flex items-center space-x-3 md:space-x-4">
            <div className="w-12 h-12 md:w-16 md:h-16 bg-amber-400 rounded-xl md:rounded-2xl flex items-center justify-center text-white text-2xl md:text-3xl shadow-lg shadow-amber-100 flex-shrink-0">
              {service.icon}
            </div>
            <div>
              ...
              <p className="text-amber-500 text-[10px] md:text-[11px] font-bold mt-1">
                {service.description}
              </p>
              <p className="text-amber-500 text-[10px] md:text-[11px] font-medium opacity-80">
        ...
        <div className="p-6 md:p-8 pt-6 overflow-y-auto flex-1">
          ...
          <ul className="space-y-2 md:space-y-3">
            {service.requirements.map((req) => (
              <li key={req.id} className="group relative flex items-center p-3 md:p-4 bg-slate-50/50 rounded-xl md:rounded-2xl border-l-[3px] border-amber-400/30 hover:border-amber-400 hover:bg-white transition-all duration-200 shadow-sm">
                ...
                <p className="ml-4 text-slate-600 text-[10px] md:text-[11px] font-medium leading-relaxed">
                  {req.label}
                </p>
              </li>
            ))}
        ...
        {/* Modal Footer */}
        <div className="p-6 md:p-8 pt-2 grid grid-cols-3 gap-2 md:gap-3 flex-shrink-0">
          <button 
            onClick={onClose}
            className="py-3 md:py-4 px-2 md:px-4 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl md:rounded-2xl font-bold text-[10px] md:text-xs transition-colors"
          >
            Tutup
          </button>
          <a 
            ...
            className="py-3 md:py-4 px-1 md:px-2 bg-[#22C55E] hover:bg-emerald-600 text-white rounded-xl md:rounded-2xl font-bold text-[9px] md:text-xs shadow-lg shadow-emerald-100 flex items-center justify-center transition-all"
          >
          ...
          <button 
            ...
            className="py-3 md:py-4 px-2 md:px-4 bg-amber-500 hover:bg-amber-600 text-white rounded-xl md:rounded-2xl font-bold text-[10px] md:text-xs shadow-lg shadow-amber-100 transition-all"
          >
            Ajukan Sekarang
          </button>
        </div>
